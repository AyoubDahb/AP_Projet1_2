...
class Intervention
{
    ...
    {
        $requete = 'SELECT * FROM intervention i, produit p WHERE i.idproduit = p.idproduit AND p.idproduit = :idproduit';
    ...
    public function updateInterventionStatut($idintervention, $statut)
    {
        ...
        $update->bindParam(":idintervention", $idintervention, PDO::PARAM_INT);
    ...
    {
        $requete = 'SELECT COUNT(*) as total FROM intervention';
        $select = $this->bdd->prepare($requete);
        $select->execute();
        return $select->fetch();
    }

    public function getClientsWithOpenTickets()
    {
        $requete = 'SELECT COUNT(DISTINCT idclient) as total FROM intervention WHERE statut IN ("en attente", "acceptee", "en cours")';
        $select = $this->bdd->prepare($requete);
        $select->execute();
        return $select->fetch();
    }

    public function getTicketsOpenMoreThanOneDay()
    {
        $requete = 'SELECT COUNT(*) as total FROM intervention WHERE statut IN ("en attente", "acceptee", "en cours") AND DATE_ADD(date_creation_intervention, INTERVAL 0 DAY) <= NOW()';
        $select = $this->bdd->prepare($requete);
        $select->execute();
        return $select->fetch();
    }
